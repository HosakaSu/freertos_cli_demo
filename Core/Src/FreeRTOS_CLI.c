  ...
  if (pxCommand == NULL) {
    ...
    for (pxCommand = &xRegisteredCommands; pxCommand != NULL;
      ...
      if ((pcCommandInput[xCommandStringLength] == ' ') ||
        ...
        if (strncmp((const char *)pcCommandInput,
...
int8_t *FreeRTOS_CLIGetOutputBuffer(void) { return cOutputBuffer; }
/*-----------------------------------------------------------*/
...
static portBASE_TYPE prvHelpCommand(int8_t *pcWriteBuffer,
  ...
  strncpy((char *)pcWriteBuffer,
          (const char *)pxCommand->pxCommandLineDefinition->pcHelpString,
          xWriteBufferLen);
  pxCommand = pxCommand->pxNext;
